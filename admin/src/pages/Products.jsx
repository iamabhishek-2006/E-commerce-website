...
import NewProduct from "../dailogs/NewProduct";
      ...
      data.map((item) => {
        if (item._id === id) {
          return {
            ...item,
            ...newData,
            category: categories.find((c) => c._id === newData.category),
          };
        }
        return item;
      })
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res2 = await fetch("http://localhost:4000/admin/category", {
          method: "GET",
          headers: {
            "Content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((item) => (
                <tr
                  key={item._id}
                  className="border border-x-0 border-gray-300"
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    <span className="text-xs">{item.mrp}</span>
                  </td>
