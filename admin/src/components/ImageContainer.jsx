...
const ImageContainer = ({ productId, limit }) => {
  ...
  const handleAddImage = (e) => {
    ...
    if (selectedFiles.length > limit) {
      alert(`you can only selected up to ${limit} images`);
      return;
    }
  ...
  return (
    <div className=" relative my-5 p-4 flex flex-col  rounded border-dashed border-2 border-gray-400 min-h-20 overflow-auto">
      <div className="absolute top-1 right-1">
        <button
          className="disabled:cursor-not-allowed bg-blue-200 rounded disabled:pointer-events-none opacity-50 "
      ...
      {files.length === 0 && (
        <p className="text-gray-600 selected-none">No files selected</p>
      )}
      {/* images list */}
