...
import type { IWishlist } from '@/types/Index';
...
const Product = (data: IWishlist) => {
  const { removeWishlistItem, addWishlistItem, moveToCart,rollbackToWishlist} =
    useUserStore();
  ...
  const handleMoveToCart = async () => {
    try {
      // data flow in client
      // moveToCart({ item: wishListObj, quantity: 1 });
      moveToCart({item:wishListObj})

      // data flow in server
      const res = await fetch(
        `http://localhost:4000/user/wishlist/move-to-cart/${data.item._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const removeFromWishlist = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:4000/user/wishlist/${data.item._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
