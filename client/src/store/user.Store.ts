import type { IAddress, ICart, IUser, IWishlist } from "@/types/Index";
...
const useUserStore = create<IUserStore>((set) => ({
  ...
  deleteAddress: (id) =>
    set((state) => ({
      addresses: state.addresses!.filter((item) => item._id !== id),
    })),
  ...
  removeCartItem: (itemId) =>
    set((state) => ({
      cart: state.cart!.filter((item) => item.item._id !== itemId),
    })),
  ...
  removeWishlistItem: (itemId) =>
    ...
    set((state) => ({
      wishlist: state.wishlist!.filter(
        (item) => (item.item._id || item.item) !== itemId
      ),
    })),
  moveToCart: (data) =>
    set((state) => {
      ...
      return {
        cart: updatedCart,
        wishlist: state.wishlist?.filter(
          (item) => item.item._id !== data.item._id
        ),
      };
    }),
